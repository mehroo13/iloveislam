...
export default function QiblaFinder() {
  ...
  // ── Ring rotates opposite to device (keeps fixed to Earth) ──
  const ringRotation = deviceHeading !== null ? -deviceHeading : 0;

  // ── CRITICAL FIX: Needle is relative when compass is active ──
  const needleRotation = 
    deviceHeading !== null && qibla !== null 
      ? ((qibla - deviceHeading + 360) % 360)
      : (qibla ?? 0);
    ...
    navigator.geolocation.getCurrentPosition(
      async ({ coords: { latitude, longitude } }) => {
        try {
          ...
          applyLoc(latitude, longitude, 'Your Location', '', '📍');
        }
      },
      (e) => {
        setLoading(false);
          ...
          <div className="space-y-4">
              ...
              <div className="w-72 h-72 sm:w-80 sm:h-80">
                <svg viewBox="0 0 300 300" className="w-full h-full">
                  ...
                  <g style={{
                    ...
                    transition: deviceHeading !== null ? 'transform 0.08s linear' : 'none',
                  }}>
                    ...
                    {[
                      ...
                      { label: 'NE', deg: 45, color: 'rgba(255,255,255,0.45)', size: 10 },
                      { label: 'E', deg: 90, color: 'rgba(255,255,255,0.85)', size: 15, bold: true },
                      { label: 'SE', deg: 135, color: 'rgba(255,255,255,0.45)', size: 10 },
                      { label: 'S', deg: 180, color: 'rgba(255,255,255,0.85)', size: 15, bold: true },
                      { label: 'SW', deg: 225, color: 'rgba(255,255,255,0.45)', size: 10 },
                      { label: 'W', deg: 270, color: 'rgba(255,255,255,0.85)', size: 15, bold: true },
                      { label: 'NW', deg: 315, color: 'rgba(255,255,255,0.45)', size: 10 },
                    ...
                    <circle cx={CX} cy={CY} r="100" fill="rgba(10,30,20,0.75)" stroke="rgba(255,255,255,0.08)" strokeWidth="1" />
                  </g>

                  {/* Needle - Now Correctly Relative */}
                  <g style={{
                    transformOrigin: `${CX}px ${CY}px`,
                    transform: `rotate(${needleRotation}deg)`,
                    transition: 'transform 0.25s ease-out',
                  }}>
                    <text x={CX} y={CY - 88} textAnchor="middle" dominantBaseline="central" fontSize="22">🕋</text>
                    <polygon
                      points={`${CX},${CY - 78} ${CX - 8},${CY - 22} ${CX + 8},${CY - 22}`}
                      fill="#c8a96e" opacity="0.95" />
                    <polygon
                      points={`${CX},${CY + 52} ${CX - 6},${CY + 12} ${CX + 6},${CY + 12}`}
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="bg-white/10 border border-white/15 rounded-2xl p-4 text-center">
                ...
                <p className="text-white/30 text-xs mt-0.5">
                  ≈ {Math.round((dist ?? 0) * 0.621371).toLocaleString()} miles
                </p>
              </div>
            </div>
            ...
            {compassState === 'active' && (
              <div className="flex items-center justify-between bg-emerald-500/20 border border-emerald-400/30 rounded-xl px-4 py-2.5">
                ...
                <button onClick={disableCompass} className="text-white/40 hover:text-white text-xs">Disable</button>
              </div>
            )}

            {(compassState === 'denied' || compassState === 'unsupported') && (
              <div className="bg-yellow-500/15 border border-yellow-400/30 rounded-xl p-4 text-center">
                <p className="text-yellow-300 text-sm">
                  Face {qibla}° ({cardinal(qibla)}) from True North
                </p>
              </div>
            )}
            ...
            <div className="bg-white/5 border border-white/10 rounded-xl p-4">
              <p className="text-white/50 text-xs text-center leading-relaxed">
                {compassState === 'active'
                  ? '🔄 Rotate your phone slowly. The golden 🕋 will point straight up when you face the Qibla.'
                  : `📐 Face ${qibla}° (${cardinal(qibla)}) from True North to face the Qibla.`}
              </p>
            </div>
